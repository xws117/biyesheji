...
def randomUserAgent():
    ...
    return ans[p]
for k in range(200,312):
    try:
        headers = {
        'Referer':'http://www.cbs.dtu.dk/services/NetOGlyc/',
        ...
        files={'configfile':(None,'/usr/opt/www/pub/CBS/services/NetOGlyc-4.0/NetOGlyc.cf'),
            'SEQPASTE':(None,''),
        ...
        if haha.text.find('1179066213@qq')!=-1:
            ...
            fp = open('C:\Users\Mark\Desktop\wenjian\sanswer\jobid.txt','a')
            fp.write(jobid)
            fp.write('\n')
            fp.close()
