...
for k in range(0,1):
    try:
        headers = {
        'Referer':'http://www.cbs.dtu.dk/services/TatP/',
        ...
        files={'configfile':(None,'/usr/opt/www/pub/CBS/services/TatP-1.0/tatp.cf'),
            'SEQPASTE':(None,''),
            'SEQSUB':('seq-%d.fasta'%(k+1),open(openfile,'rb'),'application/octet-stream'),
            'regex_f':(None,'RR.[FGAVML][LITMVF]'),
            #'SEQSUB':'',
            'graphmode':(None,'gif'),
            'format':(None,'short'),
            'trunc':(None,'100')
